...
def probabilitySegmentation2(name, img, P_positive, min_P, bmin=0, gmin=0, rmin=0, db=10, dg=10, dr=10):
    directory = "output/" + name + "/"
    if not os.path.exists(directory):
        os.makedirs(directory)
    ...
    # cv2.imshow('probabilitySegmentation', img_P)
    cv2.imwrite(directory + name + '_probability_' + str(min_P) + '.jpg', img_P)
...
if __name__ == '__main__':
    ...
    for name in gv.name_perspectives:
        ...
        probabilitySegmentation2(name, img_resize, P_positive, 0.3, 0, 0, 0, gv.d, gv.d, gv.d)
